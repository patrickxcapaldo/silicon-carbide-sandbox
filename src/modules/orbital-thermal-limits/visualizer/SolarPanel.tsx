...
import { SOLAR_HINGE, solarWingSize } from './spacecraftGeometry';
 ...
 * That is modelled here literally: the wing group rotates about its own boom
...
export function SolarPanel({ side, areaM2, pointingFactor, generationFactor }: Props) {
  ...
  const { length, width } = solarWingSize(areaM2);
  const hinge = SOLAR_HINGE; // just beyond the bus's flush end-panels
